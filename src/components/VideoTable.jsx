...
const VideoTable = () => {
  const videos = Array.from({ length: 50 }).map((_, index) => ({
    id: index + 1,
    title: "There are many variations of passages of ",
    coursename: "printing, and typesetting ,industry Lorem, Ipsum has ", // sample author image
    subcourse: "typesetting",
    view: "200",
    question: "view",
  }));
        ...
        <table className="w-[800px] md:w-full text-sm text-left">
          ...
          <tbody>
            {paginatedData.map((video) => (
              <tr key={video.id} className="bg-transparent hover:bg-[#E4E4E4]">
